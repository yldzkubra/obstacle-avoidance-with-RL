...
class Robot:
    ...
    def kinematics(self, time_step):
        ...
        d_y = self.yg - self.y0
        alpha = atan2(d_y, d_x)
        angle_difference = alpha - self.q0
        ...
        w = math.atan2(2 * 0.5 * math.sin(angle_difference), 1) * self.Kh
...
def choose_action(self, x0, y0, xg, yg, closest_obs):
        ...
        # Engelden kaçınma durumunu değerlendir
        ...
        if np.random.uniform() < epsilon:
            if distance_to_goal > 5:  # Hedefe uzaksa
                if distance_to_obstacle < 20:  # Engelden yakınsa
                    action = np.random.choice([2, 3])  # Dönme eylemlerini seç
